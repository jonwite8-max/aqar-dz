...
export class PostgresAuthorizationRepository extends AuthorizationRepository {
  ...
  async grantRole(userId: string, role: UserRole, tx?: QueryExecutor): Promise<void> {
    await this.q(tx).query(`INSERT INTO authorization.user_roles(user_id,role,scope_type,scope_key) VALUES($1,$2,'platform','*') ON CONFLICT DO NOTHING`, [userId, role]);
  }
  async listRoles(userId: string, tx?: QueryExecutor): Promise<UserRole[]> {
    const result = await this.q(tx).query<RoleRow>('SELECT role FROM authorization.user_roles WHERE user_id=$1 ORDER BY role', [userId]);
